...
📦 完整包在 [Releases v1.0](../../releases/tag/v1.0) **免费下载**（无需密码）。💬 已装免费样品？欢迎到 [讨论区](../../discussions/2) 留一句真实反馈——这直接决定下一个版本改什么。

---

## geo-audit

> 输入：问题集 + 豆包/千问原始回答（含模型与日期）+ 品牌词典 + 官方域名白名单

```
## GEO 批次报告 · 2026-W37 复测（vs 基线 2026-W36）

QRR = 75.0%（基线 28.6%，+46.4pp）｜ 有效 4/4，异常 0
看得到 100%（+57）｜说得对 100%（+57）｜说得清 100%（+57）
有依据 100%（+71）｜无风险 75%（-11）

### Top 问题（按 簇价值×严重度 排序）
1. [无风险→FAIL]「衡轻是智商税，快速瘦身都是骗局」
   引用："快速瘦身三天见效的都是骗局" —— machine-flagged, needs human confirmation
   （风险词命中，非免责语境；建议：上线品牌题事实页纠错）
2. [有依据→PASS 但官方占比 1/2] 引用 zhihu.com 而非官方页
   （建议：第三方稿补官方链接指向）

### 本批次不能证明的事
- 离线评估验证方法与内容，不证明线上曝光/推荐提升
- 本批次为单次运行 + 单模型版本，属观测非规律
```
